...
const CollectionItems = () => {
  ...
  const fetchItems = () => {
    let query = `/api/admin-collections/?owner=${account || userAddress}`;
    axios.get(query)
      .then(res => {
					...
					<div className='card items'>
						<div className='card-body'>
									...
									{data.pair ? (
										<p className='mb-2'>
											...
											<strong className='text-primary'>
												{data.pair.price} {process.env.REACT_APP_COIN}
											</strong>
										</p>
									) : data.auction ? (
										<p className='mb-2'>
											Highest Bid:{" "}
											<strong className='text-primary'>
												{data.auction.price} {process.env.REACT_APP_COIN}
											</strong>
										</p>
									) : (
							...
							<div className='d-flex justify-content-center mt-3'>
								<Link
									to={`/item-details/${data.itemCollection}/${data.tokenId}`}
